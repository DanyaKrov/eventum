...
class UserServiceImpl(
    ...
    override fun getByEmail(email: String): User = dao.findByEmail(email) ?: throw NotFoundException()
    ...
    override fun create(user: UserRequest): User {
        val entity = userMapper.createUser(user)
        user.friends.forEach { entity.friends.add(getById(it)) }
        user.events.forEach { entity.events.add(eventDao.findById(it).orElseThrow { NotFoundException()} ) }
